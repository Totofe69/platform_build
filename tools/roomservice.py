...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from Cosmic-OS Github (http://github.com/Cosmic-OS)." % device
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/cos_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'Cosmic-OS/%s already exists' % (repo_name)
            else:
    ...
    f = open('.repo/local_manifests/cos_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for Cosmic-OS/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: Cosmic-OS/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "Cosmic-OS/%s" % repo_name, "revision": "n7.1" })
...
print "Repository for %s not found in the Cosmic-OS Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/cos_manifest.xml" % device
